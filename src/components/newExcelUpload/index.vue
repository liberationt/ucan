      ...
      <OpenDailog :dialog-table-visible="dialogTableVisible" :close="closeIt" :open="ajaxData" :title="title" :width="'900px'" :height="'0'">
        <div slot="content">
          <div class="excel-content">
            <div v-if="volunteerImport">
              <p>说明：</p>
              <p>1、导入之前请先下载模板</p>
              <p>2、红色*号为必填选项，如果不填写就进行导入则导入失败；</p>
              <p>3、姓名可以不填写，系统会根据身份证号码自动录入；</p>
              <p>4、如果导入失败，系统会自动生成导入失败的记录，你可以下载此文件，根据提示，调整导入的文件，调整正确后再进行数据导入；</p>
              <p>5、数据导入成功，系统会自动生成导入成功的记录，你可以下载此文件，进行数据核对；如果有新的导入，也可以进行数据核验；</p>
              <p>6、卡片状态：使用中、注销、异常；</p>
            </div>
            <div v-else>
              <p>说明：</p>
              <p>1、导入之前请先下载模板</p>
              <p v-if="volunteerService">2、如果导入失败，系统会自动生成导入失败的记录，可下载此文件，根据提示调整后，点击导入失败数据按钮进行数据导入；</p>
              <p v-else>2、如果导入失败，系统会自动生成导入失败的记录，可下载此文件，根据提示调整后，再进行数据导入；</p>
              <p>3、如果导入成功，系统会自动生成导入成功的记录，可下载此文件，进行数据核对；</p>
            </div>
            <div class="excel-btn-file" style="width: 18%;">
              ...
              <div style="width:300px;">
                ...
                <el-upload
                  ...
                  style="display: inline-block;margin-right: 30px;float:left"
                >
                  <el-button v-show="volunteerService" type="primary" :disabled="isUploading">导入失败数据</el-button>
                  <!--<div slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过500kb</div>-->
                </el-upload>
              </div>
            </div>
...
import { searchHistory } from '@/api/importExportApi'
