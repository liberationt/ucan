		...
		<el-header class="formDiv">
			<el-form :inline="true" :model="form" class="demo-form-inline">
				...
				<el-form-item label="关键词">
					<el-input v-model.trim="form.keyword" @keyup.enter.native="onSubmit" clearable placeholder="请输入机构名称查询" style="width: 200px"
					></el-input>
				</el-form-item>
				<el-form-item>
			...
			<el-container>
				...
				<el-main>
					...
					>
						...
						<el-table-column label="机构名称" show-overflow-tooltip prop="orgName" min-width="200"></el-table-column>
						<el-table-column label="机构类型" prop="orgTypeLabel" min-width="120"></el-table-column>
						<el-table-column label="地址" show-overflow-tooltip prop="addr" min-width="200"></el-table-column>
						<el-table-column label="联系电话" prop="phone" min-width="100"></el-table-column>
						<el-table-column label="成立时间" prop="establishTime" min-width="80"></el-table-column>
				...
				<el-dialog title="机构类型选择" :visible.sync="dialogVisible" class="importStyleDialog" :close-on-click-modal="false">
					<div>
						<el-row class="instituWrap">
							<div class="instituDiv" @click="jumpPensionAgency">
								<span class="iconfont iconyljg"></span>
								<span class="text">养老机构</span>
							</div>
							<div class="instituDiv" @click="jumpElderlyCareHome">
								<span class="iconfont iconzzzh"></span>
								<span class="text">长者照护之家</span>
							</div>
						</el-row>
					</div>
				</el-dialog>
				<el-dialog title="导入养老机构信息" :visible.sync="dialogVisibleImport" class="importStyleDialog" :close-on-click-modal="false" v-dialogDrag>
					<import-dialog :upload-url="uploadUrl" :template-api="getOrdanFaultDownload" template-name="养老机构导入模板" @visible="visible" :dialog-visible="dialogVisibleImport"></import-dialog>
				</el-dialog>
			</el-container>
		</el-main>
	</el-container>
  ...
  import {downloadFromBlob} from '@/utils/index';
	...
	export default {
		...
		methods: {
			...
			toDetail(row) {
				this.$router.push({
					path: `/pensionAgency/details/${row.orgId}`,
					query: {'id': row.orgId, 'modelType': 'look', 'instituType': row.orgType,'_title': `查看${row.orgName}`,'editAuthority':this.$route.meta.id}
				});
			},
			// 多选的行数据
			...
			handleEdit(row) {
				this.$router.push({
					path: `/pensionAgency/details/${row.orgId}`,
					query: {'id': row.orgId, 'modelType': 'edit', 'instituType': row.orgType,'_title': `编辑${row.orgName}`}
				});
			},
			// 删除按钮
