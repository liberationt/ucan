  ...
  <el-container>
    <el-header class="formDiv">
      <el-form :inline="true" :model="accountForm" class="demo-form-inline">
        <el-row>
          <el-form-item v-if="isTimeShow" label="账号类型">
            <el-select
          ...
          <el-form-item v-if="data.accountType === '4'||data.accountType === '5' || data.accountType === '6' || isTimeShow" label="账号子类型">
            <el-select
              v-model="accountForm.assistiveRentType"
              clearable
              placeholder="请选择账号子类型"
              @keyup.enter.native="onSubmit"
            >
              <el-option
          ...
          <el-form-item v-if="isTimeShow" label="服务到期日">
            <el-date-picker
      ...
      <el-container>
        ...
        <el-main>
          ...
          >
            ...
            <el-table-column label="登录账号" user-id="userId" prop="loginName" min-width="160">
              <template slot-scope="{row,$index}">
                <span>{{ row.loginName }}</span>
              </template>
            </el-table-column>

            <el-table-column label="账号类型" prop="accountTypeShow" width="160" />
            <el-table-column label="显示名称" prop="userName" min-width="160" />
            <el-table-column label="所属部门" prop="fullName" min-width="250" />
            <el-table-column label="手机号码" prop="phonenumber" min-width="160" />
            <el-table-column
              v-if="isTimeShow"
              label="服务到期日"
              prop="servicePeriod"
            ...
            <el-table-column fixed="right" label="操作" width="340" header-align="center">
              <template slot-scope="{row,$index}">
                <span v-has="{class: '角色分配'}" class="table-btn" @click="roleAssignment($index, row)">
                  角色分配
                </span>
                ...
                <span v-has="{class: '重置密码'}" class="table-btn" @click="resetPassword($index, row)">
                  重置密码
                </span>
                <!--<el-button-->
                <!--type="primary"-->
...
export default {
  ...
  methods: {
    ...
    clickType() {
			  // 账号类型点击事件
      // //获取子账户
      if (this.accountForm.accountType == '6') {
        accountCheckType().then(response => {
      ...
      } else if (this.accountForm.accountType == '3') {
        // 服务商获取子账户
        accountSvDataType().then(response => {
          this.accountfjOptions = response.rows
          for (let i = 0; i < this.data.sysRoles.length; i++) {
            if (this.data.sysRoles[i].roleKey === 'order_account_type') {
              this.accountfjOptions.splice(0, 1)
              break
            }
          }
        ...
        if (response.code === 0) {
          ...
          response.rows.map(items => {
            ...
            this.accountTypeArr.map(item => {
              ...
              if (items.accountType == '3') {
                this.residenceTypeOptions.map(itemn => {
                  if (items.assistiveRentType === itemn.dictValue) {
										  if (item.dictValue === '3') {
                      items.accountTypeShow = item.dictLabel + '—' + itemn.dictLabel
                    }
              ...
              if (items.accountType === '6') {
                this.accountOrgOptions.map(itemn => {
                  if (items.assistiveRentType === itemn.dictValue) {
                    if (item.dictValue === '6') {
                      items.accountTypeShow = item.dictLabel + '—' + itemn.dictLabel
                    }
