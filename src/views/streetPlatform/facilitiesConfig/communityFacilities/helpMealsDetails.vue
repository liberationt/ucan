						...
						<el-collapse-item name="baseItem_1">
							...
							<el-form ref="form_base" key="form_base" :model="form"
								...
								<el-row :gutter="0">
									...
									<el-col :md="this.adaptiveGrid.md" :lg="this.adaptiveGrid.lg">
										<el-form-item :label="this.isEditForm?'机构类型':'机构类型：'" prop="orgTypeName">
											<el-select v-if="this.isEditForm" v-model="form.orgType" placeholder="请选择机构类型" @keyup.enter.native="editContent('form_base')"
												...
												<el-option label="助餐点" name="2014" value="2014"></el-option>
												<el-option label="社区长者食堂" name="2015" value="2015"></el-option>
											</el-select>
									...
									<el-col :md="this.adaptiveGrid.md" :lg="this.adaptiveGrid.lg">
										<el-form-item :label="this.isEditForm?'证照情况':'证照情况：'" prop="setType">
											<el-select v-if="this.isEditForm" v-model="form.certStatus" @keyup.enter.native="editContent('form_base')"
											           placeholder="请选择证照情况" clearable style="width:100%;">
												<el-option v-if="biz_cert_statusOptions.length !== 0"
												           v-for="item in biz_cert_statusOptions" :key="item.dictValue"
												           :label="item.dictLabel" :value="item.dictValue"></el-option>
											</el-select>
											<span v-else>{{form.certStatus}}</span>
										</el-form-item>
									</el-col>
									<el-col :md="this.adaptiveGrid.md" :lg="this.adaptiveGrid.lg">
					...
					<el-collapse v-model="activeCollapse">
						<el-collapse-item name="baseItem_2">
							<template slot="title">
								<div class="titleSlider">
									<span>
										<i class="titleTip"></i>最大供餐能力
									</span>
								</div>
							</template>
							<el-form ref="form_gongcang" key="form_gongcang" :model="form" :rules="this.isEditForm?rulesForm:null" label-width="180px">
								<el-row :gutter="0">
									<el-col :md="this.adaptiveGrid.md" :lg="this.adaptiveGrid.lg">
										<el-form-item :label="this.isEditForm?'集中供餐人数（人/天）':'集中供餐人数（人/天）：'" prop="focusFoodNum">
											<el-input v-if="this.isEditForm" ref="focusFoodNum" v-model="form.focusFoodNum" @keyup.enter.native="editContent('form_base')" @keyup.native="transformNumber(form.focusFoodNum,'focusFoodNum')"
											          :disabled="disabled" clearable placeholder="请输入集中供餐人数"></el-input>
											<span v-else>{{form.focusFoodNum}}</span>
										</el-form-item>
									</el-col>
									<el-col :md="this.adaptiveGrid.md" :lg="this.adaptiveGrid.lg">
										<el-form-item :label="this.isEditForm?'上门送餐人数（人/天）':'上门送餐人数（人/天）：'" prop="doorFoodDeliveryNum">
											<el-input v-if="this.isEditForm" ref="doorFoodDeliveryNum" v-model="form.doorFoodDeliveryNum" @keyup.enter.native="editContent('form_base')" @keyup.native="transformNumber(form.doorFoodDeliveryNum,'doorFoodDeliveryNum')"
											          :disabled="disabled" clearable placeholder="请输入上门送餐人数"></el-input>
											<span v-else>{{form.doorFoodDeliveryNum}}</span>
										</el-form-item>
									</el-col>
								</el-row>
							</el-form>
						...
						<template v-for="(item, index) in this.institutionField">
							<InstituField :currentItem="item" :index="index+1" :modelType="modelType" :key='index'></InstituField>
						</template>
						<!--基本服务item end-->
					</el-collapse>
				</el-tab-pane>
				...
				<el-tab-pane label="扩展信息" name="extension">
					<el-collapse v-model="activeCollapse">
						...
						<template v-for="(item, index) in this.privilegeField">
							<InstituField :currentItem="item" :index="index+3" :modelType="modelType"></InstituField>
						</template>
						<!--享受政策 end-->
	...
	export default {
		...
		methods: {
			...
			getSelect() {
				return new Promise((resolve, reject) => {
					//数据字典接口
					let dicKey = 'house_type,set_mode_type,sys_Yes_No,open_state,operate_mode_3,biz_cert_status';
							...
							for (let k in response.data) {
								switch (k) {
									...
									case 'operate_mode_3':
										...
										break;
									//证照情况
									case 'biz_cert_status':
										this.biz_cert_statusOptions = response.data[k];
										break;
								}
							}
						}
			...
			editContent(formName) {
				...
				//集中供餐人数
				if(this.form.focusFoodNum > 9999999){
					return this.$message.error("集中供餐人数输入数值不合法，请重新输入！");
				}
				//上门送餐人数
				if(this.form.doorFoodDeliveryNum > 9999999){
					return this.$message.error("上门送餐人数输入数值不合法，请重新输入！");
				}
				...
				this.$refs[formName].validate((valid, object) => {
					if (valid) {
						...
						if (this.modelType === 'add') {
							if(this.cheackSet(params.providerItemVos)){
			...
			cheackSet(providerItemVos){
				let isTrue = true
				providerItemVos.forEach((item,index)=>{
					if(item.elementType=='inputNumberCheckbox' && item.serviceValue=='其他'){
						if(item.subsidyValue==""||item.subsidyValue==null){
							this.$message.error("请输入价格！");
							this.activeTabs = 'service';
							return isTrue = false
						}
					}
				})
			...
			toEditType() {
				...
				this.$router.push({
					path: `/communityFacilities/helpMealsDetails/${_id_}`,
