						...
						<el-collapse-item name="baseItem_1">
							...
							<el-form ref="form_base" key="form_base" :model="form"
								...
								<el-row :gutter="0">
									...
									<el-col :md="this.adaptiveGrid.md" :lg="this.adaptiveGrid.lg">
										<el-form-item :label="this.isEditForm?'机构建造形式':'机构建造形式：'" prop="orgBuildForm">
											<el-select v-if="this.isEditForm" v-model="form.orgBuildForm" @keyup.enter.native="editContent('form_base')"
											           placeholder="请选择机构建造形式" clearable style="width:100%;">
												<el-option v-if="set_mode_typeOptions.length !== 0"
												           v-for="item in set_mode_typeOptions" :key="item.dictValue"
												           :label="item.dictLabel" :value="item.dictValue"></el-option>
											</el-select>
											<span v-else>{{form.orgBuildForm}}</span>
										</el-form-item>
									</el-col>
									<el-col :md="this.adaptiveGrid.md" :lg="this.adaptiveGrid.lg">
                    ...
                    <el-form-item :label="this.isEditForm?'':'纬度：'" prop="openState">
                      <span v-if="!this.isEditForm">{{form.latitude}}</span>
                        ...
                        <viewer :images="imageInfoVosLookUrl">
                          <el-col class="agencyImageCol" v-for="(item,index) in imageInfoVosLookUrl">
	...
	export default {
		...
		methods: {
			...
			getSelect() {
				return new Promise((resolve, reject) => {
					//数据字典接口
					let dicKey = 'house_type,institutional_construction,sys_Yes_No,open_state,corp_attr_2,operate_mode';
							...
							for (let k in response.data) {
								switch (k) {
									...
									case 'house_type':
										...
										break;
									//设置形式
									case 'institutional_construction':
			...
			cheackSet(providerItemVos){
				...
				providerItemVos.forEach((item,index)=>{
					if(item.elementType=='inputCheckbox' && item.serviceValue=='名称：'){
						if(item.subsidyValue==""||item.subsidyValue==null){
							this.$message.error("请输入名称！");
							this.activeTabs = 'service';
							return isTrue = false
						}else{
							if(item.subsidyValue.length>50){
								this.$message.error("名称不能超过50字，请重新输入！");
								this.activeTabs = 'service';
								return isTrue = false
							}
						}
					}
					if(item.elementType=='inputCheckbox' && item.serviceValue=='助餐形式：'){
						if(item.subsidyValue.length>20){
							this.$message.error("助餐形式不能超过20字，请重新输入！");
							this.activeTabs = 'service';
							return isTrue = false
						}
					}
