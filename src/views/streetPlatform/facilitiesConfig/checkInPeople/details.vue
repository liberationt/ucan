				...
				<el-tab-pane label="入住记录" name="base">
							...
							<el-form ref="form_base" key="form_base" :model="form"
								...
								<el-row :gutter="0">
									<el-form-item hidden prop="personId">
										<el-input v-model="form.personId" disabled></el-input>
									</el-form-item>
									<el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
										<el-form-item :label="isEditForm?'姓名':'姓名：'" prop="fullName">
											<el-input v-if="isEditForm" v-model.trim="form.fullName"
											          clearable :disabled="!!form.personId"
									...
									<el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
										<el-form-item :label="isEditForm?'身份证号':'身份证号：'" prop="idCard">
											<el-input v-if="isEditForm" v-model.trim="form.idCard"
											          ...
											          placeholder="请输入身份证号" @blur="handleIdCardBlur"></el-input>
											<span v-else>{{form.idCard}}</span>
										</el-form-item>
									</el-col>
									<el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
										<el-form-item :label="isEditForm?'性别':'性别：'" prop="sex">
											<el-select v-if="isEditForm" v-model="form.sex" :disabled="!!form.personId"
									...
									<el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
										<el-form-item :label="isEditForm?'出生日期':'出生日期：'" prop="birthday">
											<el-date-picker v-if="isEditForm" v-model="form.birthday" type="date"
											                @change="birthdayChange" placeholder="请选择出生日期"
									...
									<el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
										<el-form-item :label="isEditForm?'年龄':'年龄：'" prop="age">
											<el-input v-if="isEditForm" v-model="form.age" disabled
											          clearable placeholder="系统自动计算"></el-input>
                  ...
                  <span>
                    <i class="titleTip"></i>服务信息
							...
							<el-form ref="form_service" key="form_service" :model="form"
							         @keyup.enter.native="submitBtn"
							         :rules="isEditForm?rulesForm:null" label-width="120px">
								<el-row :gutter="0">
									<el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
										<el-form-item :label="isEditForm?'服务类型':'服务类型：'" prop="serviceType">
											<el-select v-if="isEditForm" v-model="form.serviceType" clearable
											           placeholder="请选择服务类型" style="width:100%;"
									...
									<el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
										<el-form-item :label="isEditForm?'服务机构':'服务机构：'" prop="orgId">
											<el-select v-if="isEditForm" v-model="form.orgId" clearable
											           placeholder="输入服务机构名称查询" style="width:100%;"
											           filterable remote :remote-method="getServiceOrgOptions"
											           :loading="serviceOrgLoading" :disabled="!form.serviceType">
												<el-option v-for="item in serviceOrgOptions"
												           :key="item.dictValue" :label="item.dictLabel"
												           :value="item.dictValue"></el-option>
											</el-select>
									...
									<el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
										<el-form-item :label="isEditForm?'入住时间':'入住时间：'" prop="inDate">
											<el-date-picker v-if="isEditForm" v-model="form.inDate"
											                type="date" format="yyyy年MM月dd日" value-format="yyyy-MM-dd"
											                placeholder="请选择入住时间" clearable style="width:100%;">
											</el-date-picker>
									...
									<el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
										<el-form-item :label="isEditForm?'房间床位号':'房间床位号：'" prop="roomBedNo">
											<el-input v-if="isEditForm" v-model.trim="form.roomBedNo"
											          clearable placeholder="请输入房间床位号"></el-input>
											<span v-else>{{form.roomBedNo}}</span>
										</el-form-item>
									</el-col>
									<el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
										<el-form-item :label="isEditForm?'机构服务人员':'机构服务人员：'" prop="serviceMan">
											<el-input v-if="isEditForm" v-model.trim="form.serviceMan"
											          clearable placeholder="请输入机构服务人员"></el-input>
									...
									<el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
										<el-form-item prop="servicePhone">
											<!-- 宽度设为90px，使断句更友好 -->
											<span slot="label"
											      style="display:inline-block;line-height:18px;width:90px;">{{isEditForm?'服务人员联系方式':'服务人员联系方式：'}}</span>
											<el-input v-if="isEditForm" v-model.trim="form.servicePhone"
											          clearable placeholder="请输入服务人员联系方式"></el-input>
...
<script>
	...
	export default {
						...
						{trigger: 'MANUAL_TRIGGER', validator: verifyIdCard},
						{
							trigger: 'MANUAL_TRIGGER', validator: (rule, value, callback) => {
								validateIdCard(value).then(response => {
									if (response.code === 0) {
										let params = {idCard: this.form.idCard, id: this.form.id || ''};
		...
		methods: {
			...
			//确定按钮-提交form表单
			submitBtn() {
				let formRefs = ['form_base', 'form_service'].map(key => this.$refs[key]);
