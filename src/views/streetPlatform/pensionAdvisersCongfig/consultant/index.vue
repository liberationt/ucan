    ...
    <!--table板块-->
		<el-main class="tableContainer">
          ...
          <div class="total-nav">
            ...
            <el-table-column label="所属顾问点" prop="orgName" show-overflow-tooltip></el-table-column>
            <el-table-column label="所属区域" prop="areaCodeStr" show-overflow-tooltip></el-table-column>
            <el-table-column label="职务" prop="duties"></el-table-column>
            <el-table-column label="学历" prop="eduLevelName"></el-table-column>
            <el-table-column label="劳动关系" prop="workTypeName"></el-table-column>
            <el-table-column label="从事社区工作开始时间" prop="workStartdate"></el-table-column>
	...
	export default {
      ...
      // 获取区域的值
        ...
        this.form.pageNum = 1;
			...
			// 获取表格数据
			getData(form) {
				consultantSearch(form).then(res => {
					if (res.code == 0) {
            if (res.data) {
        ...
        this.idEdit=!this.idEdit;
			...
			addTableRow() {
				this.$router.push({
					...
					query: {'modelType': 'add', '_title': '新增顾问员'} //modelType: 查看:look，编辑:edit，新增:add的类型
				});
			},
			// 双击行去详情页面
			toDetail(row) {
				this.$router.push({
					path: `/consultant/details/${row.id}`,
        ...
        let content = '<p>确定要删除【' + row.fullName + '】顾问员信息吗？</p><p>一旦被删除，将无法查看该信息。</p>';
				this.$confirm(content, '删除养老顾问员', {
          dangerouslyUseHTMLString: true,
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					removeConsultant(row.id).then(response => {
						if (response.code === 0 || response.code === '0') {
							...
							this.$message.error(`删除顾问员操作失败，失败原因是：${response.msg}`);
						}
					}).catch(() => {
						this.$message.error(`删除顾问员操作失败!`);
					});
				}).catch(() => {
					this.$message({
						type: 'info',
