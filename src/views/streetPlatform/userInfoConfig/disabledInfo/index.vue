<template>
  <el-container>
    ...
    <!-- <el-dialog title="导入残疾人信息" :visible.sync="dialogVisible" class="importStyleDialog">
      <import-dialog :upload-url="uploadUrl" :template-api="getDisabledImportTemplate" template-name="残疾人信息导入模板" :dialog-visible="dialogVisible" @visible="visible" />
    </el-dialog> -->
    <ExcelUpload :dialog-table-visible="dialogVisible" :title="'导入残疾人信息'" :template-name="'残疾人信息导入模板'" :template-api="templateApi" :upload-url="uploadUrl" :type="'残疾人信息管理'" @visible="visible" />
  </el-container>
...
import { getDisabledImportTemplate, getDisabledExport } from '@/api/importExportApi'
...
import {
  ...
  getDisabledDict,
  removeDisabled
} from '@/api/userInfoConfig/disabledInfo'
...
export default {
  ...
  data() {
    return {
      ...
      exportData: '', // 查询成功后赋值
      dialogVisible: false,
      uploadUrl: process.env.VUE_APP_BASE_API + '/biz/deformity/import?ignoreField=',
      templateApi: getDisabledImportTemplate,
    ...
    getEditAuthority(this.$route.meta.id).then(res => {
      res.forEach(item => {
        if (item.menuName === '含小区导入模板') {
          ...
          this.uploadUrl = process.env.VUE_APP_BASE_API + '/biz/deformity/import?ignoreField=' + encodeURI(data.ignoreField)
          return
        }
      })
    })
  ...
  methods: {
    ...
    getDisabledTags() {
      getDisabledDict().then(res => {
        if (res) {
          this.tagOptions = res
        }
      }).catch(() => {
        this.$message.error('获取残疾人标签字典失败！')
      })
    },
    async getData(form) {
      const arr = []
      ...
      await disabledSearch(searchArr).then(res => {
        if (res && res.rows) {
    ...
    handleEdit(row) {
      this.$router.push({
        path: `/disabledInfo/details/${row.id}`,
        query: { 'modelType': 'edit', '_title': `编辑${row.fullName}` }
      })
    },
    handleRemove(row) {
      this.$confirm('确定要删除【' + row.fullName + '】吗？信息一旦被删除，将无法显示和操作。', '删除信息', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        removeDisabled(row.id).then(response => {
          if (response.code === 0 || response.code === '0') {
            ...
            this.$message.error(`删除残疾人信息操作失败，失败原因是：` + response.msg)
          }
        }).catch(() => {
          this.$message.error(`删除残疾人信息操作失败！`)
        })
      }).catch(() => {
        this.$message({
          type: 'info',
    ...
    addTableRow() {
      this.$router.push({
        ...
        query: { 'modelType': 'add', '_title': '新增残疾人' } // modelType: 查看:look，编辑:edit，新增:add的类型
      })
    },
    importFun() {
    ...
    async exportFun() {
      ...
      this.$confirm(
        ...
        .then(() => {
          getDisabledExport(this.exportData).then(response => {
            if (response.code === 0) {
      ...
      // let confirmTitle = ''
	    //   if (this.exportData.areaCode.length === 0 && this.exportData.fullName === '' && this.tags.length === 0 && this.form.ageMin === '' && this.form.ageMax === '' && this.form.householdRegistration === '' && this.form.populationType === '' && this.form.residenceType === '') {
		  //     confirmTitle = '确认导出全部残疾人信息吗？'
	    //   } else {
		  //     confirmTitle = '确认导出筛选的残疾人信息吗？'
	    //   }
	    //   this.$confirm(
		  //     confirmTitle,
      //   '导出残疾人信息',
      ...
      //         downloadFromBlob(data, '残疾人信息列表.xlsx')
      //         this.$message.success('导出残疾人信息成功！')
      //       } else {
      //         if (response.code != 0 || response.code != '0') {
      //           this.$message.error('导出残疾人信息操作失败，失败原因是：' + response.msg)
      //         }
      //       }
      //     }).catch((error) => {
      ...
      //         }
      //         this.$message.error('导出残疾人信息操作失败，失败原因是：' + error.message)
      //       } else {
      //         this.$message.error('导出残疾人信息操作失败！')
      //       }
      //     })
      //   })
      //   .catch(() => {
