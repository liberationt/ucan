          ...
          <el-collapse v-model="activeCollapse">
            ...
            <el-collapse-item name="baseItem_3">
              ...
              >
                <el-row :gutter="0">
                  <el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
                    <el-form-item :label="isEditForm?'残疾证号':'残疾证号：'" prop="bizDeformityPropertyVo.defoCode">
                      <el-input
                        v-if="isEditForm"
                        v-model.trim="form.bizDeformityPropertyVo.defoCode"
                        clearable
                        placeholder="请输入残疾证号"
                        @keyup.enter.native="submitBtn"
                      />
                  ...
                  <el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
                    <el-form-item :label="isEditForm?'残疾人类别':'残疾人类别：'" prop="bizDeformityPropertyVo.category">
                      <el-select
                        v-if="isEditForm"
                  ...
                  <el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
                    <el-form-item :label="isEditForm?'残疾类别':'残疾类别：'">
                      <el-cascader v-if="isEditForm" v-model="form.bizDeformityPropertyVo.defoType" style="width:100%" :props="props" :options="deformityTypeOptions" clearable @change="clicks" />
...
export default {
  ...
  data() {
    return {
      ...
      jobStatusOptions: [], // 就业情况
      disabledTypeOptions: [], // 残疾人类型
      deformityTypeOptions: [], // 残疾类型
      deformityTypeOptions1: [],
      deformityLevelOptions: [], // 残疾等级
      defoStatusOptions: [], // 用户状态
      // 验证表单
      dataList2: [
        {
          id: '',
  ...
  methods: {
          ...
          for (const key in dict) {
            switch (key) {
              ...
              case 'people_category':
                ...
                break
              case 'category':
                this.disabledTypeOptions = dict[key]
                break
              case 'defo_type':
                this.deformityTypeOptions = dict[key]
                break
              case 'defo_level':
                this.deformityLevelOptions = dict[key]
                break
              case 'defo_status':
                this.defoStatusOptions = dict[key]
                break
    ...
    submitBtn() {
      ...
      validateForms(formRefs).then(() => {
        ...
        delete _form.jobStatusName
        delete _form.bizDeformityPropertyVo.categoryName
        delete _form.bizDeformityPropertyVo.defoTypeName
        delete _form.bizDeformityPropertyVo.defoLevelName
        ...
        if (_form.bizDeformityPropertyVo.defoType && _form.bizDeformityPropertyVo.defoType.length) {
          const defoType = []
          const defoLevel = []
          _form.bizDeformityPropertyVo.defoType.forEach(item => {
            defoType.push(item[0])
            defoLevel.push(item[1])
          })
          _form.bizDeformityPropertyVo.defoType = defoType.toString()
          _form.bizDeformityPropertyVo.defoLevel = defoLevel.toString()
        }
        // 残疾类别 特殊处理
        else {
    ...
    editContent() {
      this.$router.push({
        path: `/disabledInfo/details/${this.$route.params.id}`,
