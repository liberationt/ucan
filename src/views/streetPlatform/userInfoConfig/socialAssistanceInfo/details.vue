            ...
            <el-collapse-item name="baseItem_3">
              ...
              >
                <el-row :gutter="0">
                  ...
                  <el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
                    <el-form-item :label="isEditForm?'家庭类别':'家庭类别：'" prop="bizHelpPropertityVo.familyType">
                      <el-select
                        v-if="isEditForm"
                  ...
                  <el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
                    <el-form-item :label="isEditForm?'家庭人数':'家庭人数：'" prop="bizHelpPropertityVo.familyNum">
                      <el-input
                        v-if="isEditForm"
                        ref="bizHelpPropertityVo.familyNum"
                        v-model.number="form.bizHelpPropertityVo.familyNum"
                        clearable
                        placeholder="请输入家庭人数"
                        @keyup.enter.native="submitBtn"
                      />
                  ...
                  <el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
                    <el-form-item :label="isEditForm?'是否覆盖对象':'是否覆盖对象：'" prop="bizHelpPropertityVo.defoCode">
                      <el-select
                        v-if="isEditForm"
                        v-model="form.bizHelpPropertityVo.defoCode"
                        clearable
                        placeholder="请选是否覆盖对象"
                        style="width:100%;"
                        @keyup.enter.native="submitBtn"
                      >
                        <el-option
                          v-for="item in isCoveredOptions"
                          :key="item.dictValue"
                          :label="item.dictLabel"
                          :value="item.dictValue"
                        />
...
export default {
  ...
  mounted() {
    ...
    if (this.modelType === 'look') {
      ...
      // 请求当前用户信息
      getSocialAssistance(_userId_).then(response => {
  ...
  methods: {
    ...
    editGetData(id) {
      return new Promise((resolve, reject) => {
        // 请求当前用户信息
        getSocialAssistance(id).then(response => {
          this.formatData(response)
          this.idEdit = false
        }).catch(() => {
          this.$message.error('获取用户信息失败！')
    ...
    submitBtn() {
      ...
      validateForms(formRefs).then(() => {
        ...
        if (this.modelType === 'add' && !this.form.id) {
          addSocialAssistance(_form).then(response => {
            if (response.code === 0) {
              this.$message({
                type: 'success',
                message: '新增社会救助信息保存成功!'
              })
              // 关闭页面
              this.closeDetailsPage()
            } else {
              this.$message.error('新增社会救助信息操作失败，失败原因是：' + response.msg)
            }
          }).catch(() => {
            this.$message.error('新增社会救助信息操作失败!')
          })
        }
        // 编辑
        else if (this.modelType === 'edit' || (this.modelType === 'add' && this.form.id)) {
          editSocialAssistance(_form).then(response => {
            if (response.code === 0) {
              this.$message({
                type: 'success',
                message: '编辑社会救助信息保存成功!'
              })
              // 关闭页面
              this.closeDetailsPage()
            } else {
              this.$message.error('编辑社会救助信息操作失败，失败原因是：' + response.msg)
            }
          }).catch(() => {
            this.$message.error('编辑社会救助信息操作失败!')
          })
        }
      }).catch(objectList => {
    ...
    closeDetailsPage() {
      ...
      this.$router.push('/streetPlatform/userInfoConfig/socialAssistanceInfo')
    },
    // 查看详情页面，点击编辑按钮
    editContent() {
      this.$router.push({
        path: `/socialAssistanceInfo/details/${this.$route.params.id}`,
    ...
    formatData(response) {
      ...
      if (response.bizHelpPropertityVo != null) {
        this.form.bizHelpPropertityVo = response.bizHelpPropertityVo
      } else {
