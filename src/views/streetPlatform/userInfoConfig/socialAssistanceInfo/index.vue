  ...
  <el-container>
    <el-header class="formDiv">
      <el-form :inline="true" :model="form" class="demo-form-inline">
        ...
        <el-form-item label="社会救助标签">
          <el-select
            v-model="tags"
            placeholder="请选社会救助标签"
            style="width: 200px"
            multiple
            @keyup.enter.native="onSubmit"
          >
    ...
    <!-- <el-dialog title="导入社会救助信息" :visible.sync="dialogVisible" class="importStyleDialog">
      <import-dialog :upload-url="uploadUrl" :template-api="getSocialAssistanceImportTemplate" template-name="社会救助信息导入模板" :dialog-visible="dialogVisible" @visible="visible" />
    </el-dialog> -->
    <ExcelUpload :dialog-table-visible="dialogVisible" :title="'导入社会救助信息'" :template-name="'社会救助信息导入模板'" :template-api="templateApi" :upload-url="uploadUrl" :type="'社会救助信息管理'" @visible="visible" />
  </el-container>
</template>

<script>
import _ from 'lodash'
import { getSocialAssistanceImportTemplate, getSocialAssistanceExport } from '@/api/importExportApi'
...
import {
  ...
  getSocialAssistanceDict,
  removeSocialAssistance
} from '@/api/userInfoConfig/socialAssistanceInfo'
...
export default {
  ...
  data() {
    return {
      ...
      exportData: '', // 查询成功后赋值
      dialogVisible: false,
      uploadUrl: process.env.VUE_APP_BASE_API + '/biz/help/import?ignoreField=',
      templateApi: getSocialAssistanceImportTemplate,
    ...
    getEditAuthority(this.$route.meta.id).then(res => {
      res.forEach(item => {
        if (item.menuName === '含小区导入模板') {
          ...
          this.uploadUrl = process.env.VUE_APP_BASE_API + '/biz/help/import?ignoreField=' + encodeURI(data.ignoreField)
          return
        }
      })
    })
  ...
  methods: {
    ...
    getSocialAssistanceTags() {
      getSocialAssistanceDict().then(res => {
        if (res) {
          this.tagOptions = res
        }
      }).catch(() => {
        this.$message.error('获取社会救助字典失败！')
      })
    },
    async getData(form) {
      const arr = []
      ...
      let searchArr = _.cloneDeep(this.form) // 深拷贝form
      searchArr = {
        ...searchArr,
        ...transformTags(this.tags)
      }
      searchArr.areaCode = searchArr.areaCode[0] // 提交表单是 区域是一个字符串 从数组中取出改变
      await socialAssistanceSearch(searchArr).then(res => {
        if (res && res.rows) {
    ...
    handleEdit(row) {
      this.$router.push({
        path: `/socialAssistanceInfo/details/${row.id}`,
        query: { 'modelType': 'edit', '_title': `编辑${row.fullName}` }
      })
    },
    handleRemove(row) {
      this.$confirm('确定要删除【' + row.fullName + '】吗？信息一旦被删除，将无法显示和操作。', '删除社会救助信息', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        removeSocialAssistance(row.id).then(response => {
          if (response.code === 0 || response.code === '0') {
            ...
            this.$message.error(`删除社会救助信息操作失败，失败原因是：` + response.msg)
          }
        }).catch(() => {
          this.$message.error(`删除社会救助信息操作失败！`)
        })
      }).catch(() => {
        this.$message({
          type: 'info',
    ...
    addTableRow() {
      this.$router.push({
        ...
        query: { 'modelType': 'add', '_title': '新增社会救助' } // modelType: 查看:look，编辑:edit，新增:add的类型
      })
    },
    importFun() {
    ...
    async exportFun() {
      ...
      this.$confirm(
        ...
        .then(() => {
          getSocialAssistanceExport(this.exportData).then(response => {
            if (response.code === 0) {
	    ...
	    //   if (this.exportData.areaCode.length === 0 && this.exportData.fullName === '' && this.tags.length === 0 && this.form.ageMin == '' && this.form.ageMax == '' && this.form.householdRegistration === '' && this.form.populationType === '' && this.form.residenceType === '') {
		  //     confirmTitle = '确认导出全部社会救助信息吗？'
	    //   } else {
		  //     confirmTitle = '确认导出筛选的社会救助信息吗？'
	    //   }
	    //   this.$confirm(
		  //     confirmTitle,
      //   '导出社会救助信息',
      ...
      //         downloadFromBlob(data, '社会救助信息列表.xlsx')
      //         this.$message.success('导出社会救助信息成功！')
      //       } else {
      //         if (response.code != 0 || response.code != '0') {
      //           this.$message.error('导出社会救助信息操作失败，失败原因是：' + response.msg)
      //         }
      //       }
      //     }).catch((error) => {
      ...
      //         }
      //         this.$message.error('导出社会救助信息操作失败，失败原因是：' + error.message)
      //       } else {
      //         this.$message.error('导出社会救助信息操作失败！')
      //       }
      //     })
      //   })
      //   .catch(() => {
