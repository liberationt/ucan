            ...
            <el-collapse-item name="baseItem_1">
              ...
              >
                <el-row :gutter="0">
                  <el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
                    <el-form-item
                      :label="isEditForm?'居住方式':'居住方式：'"
                      prop="helpPoor.liveType"
                    >
                      <el-select
                        v-if="isEditForm"
                        v-model="form.helpPoor.liveType"
                        placeholder="请选择居住方式"
                        clearable
                        style="width:100%;"
                        @keyup.enter.native="editContent"
                      ...
                      <span v-else>{{ form.bizHelpPoorPropertityVo.liveTypeName }}</span>
                    </el-form-item>
                  </el-col>
                  <el-col :md="adaptiveGrid.md" :lg="adaptiveGrid.lg">
                    <el-form-item
                      :label="isEditForm?'扶贫类别':'扶贫类别：'"
                      prop="helpPoor.helpPoorType"
                    >
                      <el-cascader
                        v-if="isEditForm"
                        v-model="form.helpPoor.helpPoorType"
                        placeholder="请选择扶贫类别"
                        clearable
                        style="width:100%;"
                        :options="helpPoorType"
                        :props="propsTwo"
                        @keyup.enter.native="editContent"></el-cascader>
                      <span v-else>{{ form.bizHelpPoorPropertityVo.helpPoorTypeName }}<span v-if="form.bizHelpPoorPropertityVo.nursingLevelName !== null">/</span>{{form.bizHelpPoorPropertityVo.nursingLevelName}}</span>
                    </el-form-item>
                  </el-col>
                </el-row>
              </el-form>
          ...
          <el-collapse v-model="activeCollapse">
            <el-collapse-item name="baseItem_5">
              <template slot="title">
                <div class="titleSlider">
                  <span>
                    <i class="titleTip" />扶贫补贴
                  </span>
                </div>
              </template>
              <el-form :model="form">
                <el-table :data="helpPoorList" style="width: 100%">
                  <el-table-column label="补贴名称" prop="subsidyElementName" />
                  <el-table-column label="类别">
                    <template slot-scope="scope">
                      <el-form-item>
                        <el-select
                          v-if="scope.row.subsidyElementName=='居家城市补贴'"
                        ...
                        <el-select
                          v-else-if="scope.row.subsidyElementName=='护理补贴'"
                          v-model="scope.row.ageRange"
                          :disabled="modelType === 'look'"
                          clearable
                          placeholder="请选择"
...
import { addPoor, editPoor, getDetailData } from '@/api/userInfoConfig/helpPoor'
...
export default {
      ...
      helpPoorDataList: [
        {
          id: '',
          subsidyDate: '',
          subsidyElementName: '居家城市补贴',
          subsidyMoney: '',
          subsidyTitleName: '扶贫补贴',
          subsidyUnit: '',
          subsidyRemark: '',
          ageRange: ''
        },
  ...
  methods: {
    ...
    transformNumber6() {
      this.helpPoorDataList = this.helpPoorList
      this.helpPoorDataList[index][obj] = transformFloatNumber1(value)
      this.$set(this.helpPoorList, index, this.helpPoorDataList[index])
    },
    getMoney(row) {
      var helpPoorList = [
        {
      ...
      if (row.subsidyElementName === '居家城市补贴') {
        if (row.ageRange === 'live_city') {
          row.subsidyMoney = 200
          row.subsidyUnit = 'yuan/month'
        } else if (row.ageRange === 'live_country') {
          row.subsidyMoney = 100
          row.subsidyUnit = 'yuan/month'
        }
      }
      if (row.subsidyElementName === '护理补贴') {
        if (row.ageRange === '1') {
          row.subsidyMoney = 100
          row.subsidyUnit = 'yuan/month'
    ...
    getSelect() {
      ...
      return new Promise((resolve, reject) => {
        allSelectdictionaryData(dictKey).then(response => {
          ...
          for (const key in dict) {
            switch (key) {
              ...
              case 'git_subsidy':
                ...
                break
              case 'live_type':
                this.liveType = dict[key]
                break
              case 'help_poor_type':
                this.helpPoorType = dict[key]
                break
              case 'nursing_level':
          ...
          this.helpPoorType[2].children = this.nursingLevel
    ...
    editContent() {
      ...
      validateForms(formRefs).then(() => {
        ...
        this.helpPoorList.forEach(item => {
          if (item.subsidyMoney !== '' && item.subsidyMoney !== null || item.subsidyRemark !== '' && item.subsidyRemark != null) {
            arrList.push(item)
          }
        })
        _form.personSubsidyInfoList = arrList
        if (_form.helpPoor.helpPoorType !== '') {
          if (_form.helpPoor.helpPoorType.length === 1) {
            _form.bizHelpPoorPropertityVo.helpPoorType = _form.helpPoor.helpPoorType[0]
            _form.bizHelpPoorPropertityVo.nursingLevel = ''
          } else if (_form.helpPoor.helpPoorType.length === 2) {
            _form.bizHelpPoorPropertityVo.helpPoorType = _form.helpPoor.helpPoorType[0]
            _form.bizHelpPoorPropertityVo.nursingLevel = _form.helpPoor.helpPoorType[1]
          } else if (_form.helpPoor.helpPoorType.length === 0) {
            _form.bizHelpPoorPropertityVo.nursingLevel = ''
            _form.bizHelpPoorPropertityVo.helpPoorType = ''
          }
        }
        _form.bizHelpPoorPropertityVo.liveType =  _form.helpPoor.liveType
        // 新增
    ...
    closeDetailsPage() {
      ...
      this.$router.push('/streetPlatform/userInfoConfig/helpPoorInfo')
    },
    // 查看详情页面，点击编辑按钮

    toEditType() {
      this.$router.push({
        path: `/helpPoorInfo/details/${this.$route.params.id}`,
    ...
    formatData(response) {
      ...
      //   }
      // }
      if (response.helpPoor === null || !response.helpPoor) {
        this.form.helpPoor = {
          helpPoorType: '',
          helpPoorTypeName: '',
          id: '',
      ...
      if (response.bizHelpPoorPropertityVo !== null) {
        this.form.helpPoor.liveType = response.bizHelpPoorPropertityVo.liveType
        if (response.bizHelpPoorPropertityVo.helpPoorType && response.bizHelpPoorPropertityVo.helpPoorType !== '') {
          this.form.helpPoor.helpPoorType = []
          this.form.helpPoor.helpPoorType.push(response.bizHelpPoorPropertityVo.helpPoorType)
          if (response.bizHelpPoorPropertityVo.nursingLevel && response.bizHelpPoorPropertityVo.nursingLevel !== '') {
            this.form.helpPoor.helpPoorType.push(response.bizHelpPoorPropertityVo.nursingLevel)
          }
        }
        this.form.bizHelpPoorPropertityVo = response.bizHelpPoorPropertityVo
      }
      if (response.bizHelpPoorPropertityVo == null) {
        this.form.bizHelpPoorPropertityVo = {
          helpPoorType: '',
          helpPoorTypeName: '',
          id: '',
          liveType: '',
      ...
      if (response.personSubsidyInfoList.length) {
        ...
        response.personSubsidyInfoList.forEach(item => {
          ...
          if (item.subsidyTitleName == '扶贫补贴') {
            helpPoorList.push(item)
          }
        })
        this.helpPoorList.forEach(item => {
          helpPoorList.forEach(items => {
        ...
        this.helpPoorDataList = _.cloneDeep(this.helpPoorList)
        const disposableTab = []
